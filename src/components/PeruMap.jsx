...
function getColor(p) {
  ...
  return SCALE.find((s) => p >= s.min)?.color ?? "rgba(255,255,255,0.06)";
}
...
export default function PeruMap({ regionData = {}, onSelectRegion }) {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-base font-semibold text-white/85 mb-3">Mapa Electoral</h2>
      ...
      >
        {geoData ? (
          ...
          <div className="flex items-center justify-center" style={{ height: "690px" }}>
            Cargando mapa…
          </div>
        )}
        ...
        {tooltip && (
          <div
            ...
            className="absolute top-3 right-3 bg-black/75 border border-white/20 rounded-xl p-3.5 shadow-2xl z-50 min-w-[160px] pointer-events-none"
          >
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <div className="flex items-center justify-between gap-6 pt-1.5 border-t border-white/10">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${ESTADO_DOT[tooltip.data.estado] ?? "bg-slate-400"}`}
                    />
                    <span className="font-medium text-white/80">{tooltip.data.estado}</span>
                  </div>
                </div>
              </div>
            ) : (
